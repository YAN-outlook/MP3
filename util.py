...
def load_adj(adj_filename):
    import os
    file_ext = os.path.splitext(adj_filename)[-1].lower()
    if file_ext == '.pkl':
        adj_mx = load_pickle(adj_filename)
    else:
...
def load_dataset(args):
    ...
    for category in ['train', 'val', 'test']:
        data[category] = np.load(os.path.join(args.data_path, category + '.npy'))
    ...
    data['train_loader'] = DataLoader(data['train'], args.batch_size, args.input_length, args.output_length)
    data['val_loader'] = DataLoader(data['val'], args.batch_size, args.input_length, args.output_length)
    data['test_loader'] = DataLoader(data['test'], args.batch_size, args.input_length, args.output_length)
    data['scaler'] = scaler
    return data
