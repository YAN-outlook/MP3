...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.gmsdr_layers = nn.ModuleList(
            [GMSDRCell(self.rnn_units, self.input_dim, adj_mx, self.max_diffusion_step, self.num_nodes, self.pre_k, self.pre_v,
...
class GMSDRModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, device, adj_mx, use_plugin=True, **model_kwargs):
        super().__init__()
        Seq2SeqAttrs.__init__(self, adj_mx, **model_kwargs)
    ...
    def forward(self, inputs, labels=None, batches_seen=None,feat=None):
        """
        ...
        #[B,N,T]
        if self.use_plugin:
            inputs = self.lin_text(inputs[:,:,:,0].unsqueeze(-1)).transpose(1,2)
            inputs = self.fusion(feat, inputs)
            #[B,T,N,D]
            B, T, N, D = inputs.shape
            # [B, T, N, D] → [T, B, N, D]
            inputs = inputs.permute(1, 0, 2, 3).contiguous()
            # [T, B, N, D] → [T, B, N*D]
            inputs = inputs.reshape(T, B, N * D)
        else:
            inputs = inputs[:,:,:,0].squeeze(-1).permute(2,0,1).contiguous()
            B, T, N = inputs.shape
            inputs = inputs.reshape(T, B, N)
