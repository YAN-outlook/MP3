...
parser.add_argument('--data_path',type=str,default='data/PEMS03/short_term',help='data path')
parser.add_argument('--adjdata',type=str,default='data/PEMS03/adj_PEMS03.pkl',help='adj data path')
...
parser.add_argument('--num_nodes',type=int,default=358,help='number of nodes')
...
parser.add_argument('--checkpoint',type=str,default='checkpoint_PEMS03_good/testA_best.pth',help='save path')
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            if args.use_long:
                ...
                for i in range(trainx.shape[0]):
                    with torch.no_grad():
                        _, feat_sample = feat_extractor(trainx[[i],:,:,:],None)
        ...
        for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            if args.use_long:
                ...
                for i in range(testx.shape[0]):
                    with torch.no_grad():
                        _, feat_sample = feat_extractor(testx[[i],:,:,:],None)
                    feat.append(feat_sample)
                feat = torch.cat(feat, dim=0)
